...
public class MobIndicatorRenderer {
	...
	public static void render(
		...
		if (dmg != null) {
			...
			if (elapsed >= 1f) {
				...
				drawColoredText(mc, poseStack, bufferSource, packedLight, dmg.text, textX, (int) floatY, 0xFFFF5555, 0xFF3F1515, cfg.showTextShadow);
			}
		}
		...
		if (heal != null) {
			...
			if (elapsed >= 1f) {
				...
				int   textX  = heartX - GAP - textW;
				drawColoredText(mc, poseStack, bufferSource, packedLight, heal.text, textX, (int) floatY, 0xFF55FF55, 0xFF153F15, cfg.showTextShadow);
			}
		}
	...
	private static void drawText(
		...
		if (ModConfig.get().showTextShadow) {
			...
			mc.font.drawInBatch(text, x + 1, y + 1, 0xFF3F3F3F, false,
					poseStack.last().pose(), bufferSource, Font.DisplayMode.NORMAL, 0, packedLight);
			poseStack.popPose();
		}
		mc.font.drawInBatch(text, x, y, 0xFFFFFFFF, false,
				poseStack.last().pose(), bufferSource, Font.DisplayMode.NORMAL, 0, packedLight);
	}
