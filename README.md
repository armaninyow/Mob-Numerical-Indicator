[![Buy Me A Coffee](https://img.shields.io/badge/Buy%20Me%20a%20Coffee-ffdd00?style=for-the-badge&logo=buy-me-a-coffee&logoColor=black)](https://www.youtube.com/watch?v=xvFZjo5PgG0)

# Mob Numerical Indicator

![Mod Icon](common/src/main/resources/assets/mobnumericalindicator/icon.png)

## Installation

* [Modrinth](https://modrinth.com/mod/mob-numerical-indicator)
* [CurseForge](https://www.curseforge.com/minecraft/mc-mods/mob-numerical-indicator)

## Support
  
If you encounter bugs or wish to contribute:
* [Report any problems you find.](https://github.com/armaninyow/Mob-Numerical-Indicator/discussions/categories/issues)
* [Share your ideas for new features.](https://github.com/armaninyow/Mob-Numerical-Indicator/discussions/categories/suggestions)

## Changelog
<details>
  <summary></summary>
  
### 2.1.0—26.x
* Added support for Minecraft 26.2
* Fixed Show Armor At Zero setting showing the armor indicator on mobs that cannot wear armor (Bat, Pufferfish, etc.)
### 2.0.0—26.x
* Added support for Minecraft 26.1, 26.1.1, and 26.1.2
* Replaced Cloth Config with YetAnotherConfigLib (YACL) 3.9.3 for the in-game config screen
* Added effect indicators that show active status effects and their remaining duration above the health row
* Added a Show Always toggle that forces indicators to appear on all mobs at all times
* Added a Y Offset slider to move the indicators up or down
### 1.0.0—1.21.x
* Initial Release
</details>

[![Buy Me A Coffee](https://img.shields.io/badge/Buy%20Me%20a%20Coffee-ffdd00?style=for-the-badge&logo=buy-me-a-coffee&logoColor=black)](https://www.youtube.com/watch?v=xvFZjo5PgG0)
